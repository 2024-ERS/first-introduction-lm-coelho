...
dat_month |> ggplot(aes(x=mdate,y=avgtemp_mo_oC)) +
  geom_line() +
  tidyquant::geom_ma(ma_fun=SMA,n=61, col="red", linetype="solid") + #n= higher 
  coord_x_date(xlim=c("2000-01-01", "2024-08-01")) + #zoom this time period
...
dat_month |> ggplot(aes(x=mdate,y=totrain_mo_mm)) +
  geom_line() +
  tidyquant::geom_ma(ma_fun=SMA,n=13, align="right", col="red", linetype="solid") + #n= number of periods
  coord_x_date(xlim=c("2014-01-01", "2024-08-01")) + #zoom this time period
...
montly_temps <- dat_month |>
  ungroup() |>
  dplyr::filter(year!=2024) |>
  dplyr::select(avgtemp_mo_oC)

plot(montly_temps$avgtemp_mo_oC)

montly_temps_ts <- ts(montly_temps, frequency = 12, start = c(1992,1))
plot(montly_temps_ts)

montly_temps_decomp <- decompose(montly_temps_ts)
plot(montly_temps_decomp)
